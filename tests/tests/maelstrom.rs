...
fn test_unique_ids() -> color_eyre::Result<()> {
    let status = Command::new("../maelstrom/maelstrom")
        .args([
            ...
            "unique-ids",
            "--bin",
            "../target/release/maelstrom-unique-ids",
            "--time-limit",
            "30",
            "--rate",
            "100",
...
fn test_single_node_broadcast() -> color_eyre::Result<()> {
    let status = Command::new("../maelstrom/maelstrom")
        .args([
            ...
            "broadcast",
            "--bin",
            "../target/release/maelstrom-broadcast",
            "--node-count",
            "1",
            "--time-limit",
            "20",
...
#[test]
fn test_broadcast_efficiency_2() -> color_eyre::Result<()> {
    let output = Command::new("../maelstrom/maelstrom")
        .args([
            "test",
            "-w",
    ...
    let messages_per_operation = edn[":net"][":all"][":msgs-per-op"]
        .to_float()
        .expect("Cast msgs-per-op to float");
    assert!(messages_per_operation < 20.0);
    let median_latency = edn[":workload"][":stable-latencies"]["0.5"]
        .to_float()
        .expect("Cast median latency to float");
    assert!(median_latency < 1000.0);
    let maximum_latency = edn[":workload"][":stable-latencies"]["1"]
        .to_float()
        .expect("Cast maximum latency to float");
...
fn test_grow_only_counter() -> color_eyre::Result<()> {
    let status = Command::new("../maelstrom/maelstrom")
        .args([
            ...
            "g-counter",
            "--bin",
            "../target/release/maelstrom-counter",
            "--node-count",
            "3",
            "--time-limit",
            "20",
...
fn test_single_node_kafka() -> color_eyre::Result<()> {
    let status = Command::new("../maelstrom/maelstrom")
        .args([
            ...
            "kafka",
            "--bin",
            "../target/release/maelstrom-kafka",
            "--node-count",
            "1",
            "--concurrency",
            "2n",
